...
a = stats.lognorm.rvs(loc = 1, s = 0.75, size = 1000)
b = stats.lognorm.rvs(loc = 0.5, s = 0.75, size = 1000)
...
#%%
final_teams_df = pd.read_csv("..//initial_code//nba_team_historical_percentiles.csv")
...
#%%
p_wrap = (
    ggplot(final_teams_df, aes(x = "WinPCT", fill = "Season")) +
...
p_wrap.save("../project_diary/figs/all_season_densities.pdf")

p_all = (
    ggplot(final_teams_df, aes(x = "WinPCT", color = "Season")) +
    geom_density()
)
p_all.save("../project_diary/figs/all_season_densities_all_together.pdf")
p_all.show()
...
p_together = (
    ...
    geom_density(color = "black", size = 2, alpha = 0.7)
    + xlim(0, 1)
)
...
#%%
current_system_data = final_teams_df[final_teams_df["system"] == "Current"]
current_system_data["WinPCT"].to_csv("current_system_win_pct_series.csv", index = False)

old_system_data = final_teams_df[final_teams_df["system"] == "Old"]
old_system_data["WinPCT"].to_csv("old_system_win_pct_series.csv", index = False)

from scipy.stats import wasserstein_distance

wass = wasserstein_distance(current_system_data["WinPCT"], old_system_data["WinPCT"])
print(f"Wassertein metric between old and new system: {wass:.4f}")
...
gini_current = calculate_gini(current_system_data["WinPCT"])
gini_old = calculate_gini(old_system_data["WinPCT"])

print(f"Gini of current system = {gini_current}")
print(f"Gini of old system = {gini_old}")
